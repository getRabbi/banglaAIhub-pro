...
export default function PromptDetail({ params }: { params: { slug: string } }) {
  ...
  if (prompt.error) return <div className="max-w-2xl mx-auto px-4 py-20 text-center text-gray-500">প্রম্পট পাওয়া যায়নি</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      ...
      <h1 className="text-2xl font-extrabold text-white mb-4">{prompt.title_bn || prompt.title}</h1>
      <p className="text-gray-400 mb-6">{prompt.description_bn}</p>
      {(prompt.use_case_bn || prompt.use_case) && <div className="glass-card p-4 mb-6"><h3 className="text-sm font-semibold text-gray-400 mb-2">📋 ব্যবহার</h3><p className="text-gray-300 text-sm">{prompt.use_case_bn || prompt.use_case}</p></div>}
