...
export async function GET(_req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  const c = COLORS[post?.category || "tech-news"] || COLORS["tech-news"];
  return new ImageResponse(
    (<div style={{ width: 1200, height: 630, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 60, background: `linear-gradient(135deg, ${c.bg} 0%, #0a0a0f 100%)`, fontFamily: "sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ padding: "8px 20px", borderRadius: 999, border: `2px solid ${c.ac}`, color: c.ac, fontSize: 22 }}>{(post?.category || "").replace("-", " ").toUpperCase()}</div></div>
