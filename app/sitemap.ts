...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [{ data: blogs }, { data: tools }, { data: cats }] = await Promise.all([
    sb.from("blog_posts").select("slug, published_at").eq("status", "published").order("published_at", { ascending: false }),
  ...
  const blogPages = (blogs || []).map(p => ({ url: `${base}/blog/${p.slug}`, lastModified: new Date(p.published_at), changeFrequency: "weekly" as const, priority: 0.7 }));
