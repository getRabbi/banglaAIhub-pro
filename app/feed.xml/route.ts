...
export async function GET() {
  ...
  const { data: posts } = await createServerClient().from("blog_posts").select("bangla_title, bangla_hook, blog_slug, category, published_at").eq("status", "published").order("published_at", { ascending: false }).limit(50);
