...
export default async function AdminComparisons() {
  const { data: items } = await createServerClient().from("comparisons").select("*").order("created_at", { ascending: false });
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-6">⚖️ Comparisons</h1>
          ...
          <tbody>
            {(items || []).map((i: any) => (
              <tr key={i.id} className="border-b border-brand-border/50">
                ...
                <td className="px-4 py-3"><Link href={`/compare/${i.slug}`} target="_blank" className="text-xs text-brand-electric hover:underline">View</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
        {(!items || items.length === 0) && <p className="text-center text-gray-600 py-10">কোনো comparison নেই</p>}
      </div>
    </div>
  );
}
