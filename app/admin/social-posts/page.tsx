...
export default async function AdminSocialPosts() {
  const { data: posts } = await createServerClient().from("social_posts").select("*, blog_posts(bangla_title, blog_slug)").order("created_at", { ascending: false }).limit(50);
          ...
          <tbody>
            {(posts || []).map((p: any) => (
              <tr key={p.id} className="border-b border-brand-border/50 hover:bg-white/[0.02]">
                <td className="px-4 py-3 text-gray-300 text-xs max-w-[200px] truncate">{p.blog_posts?.bangla_title || "-"}</td>
